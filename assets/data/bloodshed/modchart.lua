...
function update (elapsed)
    ...
    if funnywindow then
        setWindowPos(187 * math.sin(currentBeat * math.pi) + 327, 187 * math.sin(currentBeat * 3) + 160)
    end
    if funnywindowsmall then
        setWindowPos(34 * math.sin(currentBeat * math.pi) + 327, 34 * math.sin(currentBeat * 3) + 160)
    end
    if NOMOREFUNNY then
    ...
    if daNoteMove then
        for i=0,7 do
            ...
            setActorY(defaultStrum0Y + 75 * math.cos((currentBeat + i*2.8) * math.pi), i)
        end
    end
end
-- fixed the step they start at BECAUSE CYBER'S A IDIOT AND OFFSET ALL OF THEMs
